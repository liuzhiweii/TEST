...
/* 根据系统资源是否丰富，决定类型注册器的结构定义 */
#ifdef USP_LTK_RICH_RESOURCE
struct LLRP_STypeRegistry
{
...
struct LLRP_STypeRegistry
{
    const LLRP_tSTypeDescriptor  *apMessageTypeDescriptors[LTKC_MAX_MESSAGE];
    const LLRP_tSTypeDescriptor  *apParameterTypeDescriptors[LTKC_MAX_PARAMETER];
    unsigned int                 nMessageTypeDescriptors;
    unsigned int                 nParameterTypeDescriptors;
};
#endif
