...
typedef struct
{
    ...
    /* 清点是否结束标志, 0：未结束；1：结束；*/
    unsigned short int wResult;      /* 本消息中的标签数目；或者错误码  */
...
typedef struct
{
    ...
    unsigned short int wChannelIndex; 
    unsigned short int wRssi;
    unsigned short int wResult;              /* 操作结果，0：成功；其他Error Code */
    unsigned short int wResultLength;        /* 读取长度，错误时填0 */
    unsigned char   aucReadData[0];          /* 变长标签数据的第一个数据字段 */
...
typedef struct
{
    ...
    unsigned short int wResultLength;          /* 读取长度，错误时填0 */
    unsigned short int wResult;                     /* 操作结果，0：成功；其他Error Code*/
    unsigned char   aucLockData[0];        /* 变长标签数据的第一个数据字段 */
...
typedef struct
{
    ...
    unsigned short int wRssi;
    unsigned short int wResult;         /* 操作结果，0：成功；其他Error Code */
    unsigned short int wResultLength;         /* 结果信息长度 */
    unsigned char   aucLockQueryData[0];   /* 变长标签数据的第一个数据字段 */
