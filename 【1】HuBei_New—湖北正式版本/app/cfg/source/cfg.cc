...
static int set_eeprom_cfg_to_object()
{
    ...
    return 0;
    /* 读取nvram */
    //rc = usp_drv_eeprom_rcv(0x10, sizeof(eeprom_), (unsigned char *)&eeprom_, &dwLen);
    if (0 != rc || sizeof(eeprom_) != dwLen)
    {   
        //CMP_EXCEPT(USP_OAM_CMP_READ_EEPROM_ERR, dwRet, dwLen, 0);
        return rc;   
    }
    ...
    json_object_set_boolean(cur_object,"IsDhcp",eeprom_.is_dhcp);
...
static int set_object_value_to_eeprom()
{
    ...
    /* 解析合并配置 */
    cur_object = json_object_get_object(root_object,"eeprom");
    eeprom_.is_dhcp=(unsigned char)json_object_get_boolean(cur_object,"IsDhcp");
    ...
    /* 写入nvram */
    //rc = usp_drv_eeprom_snd(0x10, sizeof(eeprom_), (unsigned char *)&eeprom_, &dwLen);
    if (0 != rc || sizeof(eeprom_) != dwLen)
    {   
        return rc;   
    }
...
void send_config_change_msg(const char *content)
{
    ...
    CFG_SEND_MSG(pub_socket,send_buf,strlen(send_buf)+1);
    CFG_DEBUG("pub_socket send over\r\n");
}
...
void deal_uhf_request_config(const char *content)
{
    ...
    CHECK_NULL_POINTER_RETURN_VOID(rfs_value);
